...
#define INVSENS_DATA_BUFFER_MAX (0xF)
#define INVSENS_DATA_ITEM_MAX (INV_FUNC_NUM)
#define INVSENS_PACKET_HEADER (0x4956) /*IV*/
#define INVSENS_COPY_BUFFER_SIZE (1024)
...
struct invsens_driver_t {
	...
	int (*preenable) (struct invsens_driver_t * drv, u32 func_mask,
			   bool enabled, struct invsens_sm_ctrl_t * sm_ctrl);
	int (*postenable) (struct invsens_driver_t * drv, u32 func_mask,
			   bool enabled, struct invsens_sm_ctrl_t * sm_ctrl);
