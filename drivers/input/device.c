...
static struct base_info sensors[] = {
        ...
        {   "dmard10", {0x18                  }, 0xff, {0xff          }},
        {   "dmard06", {0x1c                  }, 0x0f, {0x06          }},
        {   "mxc622x", {0x15                  }, 0xff, {0xff          }},    
...
static int write_device_info(void)
{
        ...
        filp = filp_open(FILE_DIR, O_WRONLY | O_CREAT , 0666);
        if(!filp || IS_ERR(filp)){
                 printk("%s:open error ....IS(filp):%ld\n", __func__,IS_ERR(filp));
                return -1;
        } 
...
static int chip_id_detect_gsensor(struct base_info *info, int i2c_address_number)
{
        int detect_value = 0;
        int ret = -1, i = 0;
...
*********************************************************************************************************
*/
static int chip_id_detect_ctp(struct base_info *info, int i2c_address_number)
{
        int detect_value = 0;
        int ret = -1, i = 0;
        unsigned short temp_addr;
        ...
        int address_number, enum twi_device_type type )
{
        ...
        while(scan_number < (support_number)){
        	...
        	case TWI_GSENSOR://gsensor device
        	        report_value = chip_id_detect_gsensor(info, i2c_address_number);
        	        break;
        	case TWI_CTP://ctp device
                        report_value = chip_id_detect_ctp(info, i2c_address_number);
        	        break;
        	}
